...
def resize_single(image: np.ndarray, img_size: int = 640) -> np.ndarray:
    ...
    return cv2.copyMakeBorder(resized, 0, pad_h, 0, pad_w, cv2.BORDER_CONSTANT, value=(0, 0, 0))

def resize(image: np.ndarray, img_size: int = 640, expand_dims: bool = False) -> np.ndarray:
    ...
    if len(image.shape) == 4:
        out = np.stack([resize_single(image[i], img_size) for i in range(image.shape[0])])
        return out
    else:
...
def normalize(image: np.ndarray, mean: tuple[float, float, float] = (0.485, 0.456, 0.406), std: tuple[float, float, float] = (0.229, 0.224, 0.225), max_pixel_value=255.0) -> np.ndarray:
    """
...
def get_priorbox(image_size: tuple[int, int] = (640, 640), min_sizes: list[list[int]] = None, steps: list[int] = None, clip: bool = False) -> np.ndarray:
...
def _get_reference_facial_points(output_size:Optional[Tuple[int, int]]=None, default_square:bool=False) -> np.ndarray:
...
def align_face(img: np.ndarray, landmarks: Union[np.ndarray, Dict[str, Any]], align_size: Tuple[int, int] = (112, 112)) -> np.ndarray:
    ...
    ref_pts = _get_reference_facial_points(output_size=align_size, default_square=default_square)
