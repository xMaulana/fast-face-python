...
from .tools import resize, normalize, get_priorbox, decode, decode_landmark, nms, parse_det
...
class Inference:
    def __init__(
        ...
        if model == "MOBILENET":
            model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "mobilenet_retinaface.onnx")
        else:
            model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "resnet50_retinaface.onnx")
        self.sessions = ONNXSession(providers=providers, model_path=model_path, sess_options=sess_options)
    ...
    def preprocess(self, imgs: np.ndarray):
        ...
        imgs = normalize(imgs, mean=(104.0, 117.0, 123.0), std=(1.0, 1.0, 1.0), max_pixel_value=1.0)
    ...
    def detect(self, imgs: Union[str, List[str], np.ndarray, List[np.ndarray]], return_dict: bool = False) -> List[Union[np.ndarray, List[Dict[str, Any]]]]:
        ...
        for i in range(loc.shape[0]):
            dets = self.post_process(loc[i], conf[i], landms[i], original_shapes[i], (h, w))
            if return_dict:
    ...
    def post_process(self, loc: np.ndarray, conf: np.ndarray, landms: np.ndarray, original_shape: Tuple[int, int], preprocessed_shape: Tuple[int, int]) -> np.ndarray:
