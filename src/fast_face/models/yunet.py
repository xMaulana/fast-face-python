...
class YuNet(BaseFaceModel):
    ...
    def __init__(self,
        model_path: str = os.getenv("FAST_FACE_DIR_PATH", os.path.join(os.path.dirname(os.path.abspath(__file__)), "yunet.onnx")),
    ...
    def post_process(self, outputs: List[np.ndarray], original_shapes: List[Tuple[int, int]], preprocessed_shape: Tuple[int, int]) -> List[np.ndarray]:
        ...
        for i in range(batch_size):
            ...
            for k in range(5):
                decoded_kps[:, k*2:k*2+1] = batch_kps[:, k*2:k*2+1] * priors[:, 2:3] + priors[:, 0:1]
                decoded_kps[:, k*2+1:k*2+2] = batch_kps[:, k*2+1:k*2+2] * priors[:, 3:4] + priors[:, 1:2]
