...
def test_yunet_batched_inference():
    model_path = os.getenv(
        "FAST_FACE_DIR_PATH", 
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "../src/fast_face/models/yunet.onnx")
    )
    ...
    assert isinstance(results[0], list), "Result should be a list of dicts when return_dict=True"
    assert isinstance(results[1], list), "Result should be a list of dicts when return_dict=True"
